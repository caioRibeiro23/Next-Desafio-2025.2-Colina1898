...
export default function Resumo({ quantidade }: { quantidade: number }) {
    const [frete, setFrete] = useState<number|null>(null);
    const [desconto, setDesconto] = useState<number|null>(0);
    const subtotal = 399.90;    
    const total = subtotal + (frete ? frete : 0) - (desconto ? desconto : 0);
    return (
        <div className="bg-cinzaEscuro flex flex-col items-start justify-center gap-2.5 px-4 py-2.5 w-50 bp-840:w-80 lg:w-125 border border-dourado rounded-2xl">
            ...
            <TextoResumo titulo="Total" valor={`R$ ${total.toFixed(2)}`} />
        </div>
    );
}