...
export default function CampoForm({ campo, type, id, name }: CampoFormProps) {
    return (
        <div className="flex flex-col w-full items-start justify-center font-cormorant-sc text-white md:pr-7.5">
            ...
            <div className="w-full px-1.25 py-2.5 border-b border-dourado font-medium text-start text-sm bp-540:text-base md:text-lg lg:text-xl xl:text-2xl 2xl:text-3xl 3xl:text-4xl">
                {type === "textarea" ? (
                    <textarea rows={3} id={id} name={name} required={true} className="border-none focus:outline-none w-full"></textarea>
                ) : (
                    <input type={type} id={id} name={name} required={true} className="border-none focus:outline-none w-full"/>
                )}
            </div>
        </div>
    );
